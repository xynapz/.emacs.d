...
| =C-c l ==   | Format buffer    |
| =C-c l e=   | List errors (Consult)|
| =C-c l E=   | List errors (Tree)   |
...
- **Inlay Hints**: Shows type hints and parameter names inline (like VSCode).
- **Code Lens**: Shows reference counts above functions.

** Symbols Sidebar (Treemacs)

Use =M-x lsp-treemacs-symbols= to view the outline of the current file.
Use =M-x lsp-treemacs-errors-list= to view project errors.

** Debugging (DAP)

Debugging is provided by =dap-mode=.
- Start: =M-x dap-debug=
- Breakpoint: =M-x dap-breakpoint-toggle=
